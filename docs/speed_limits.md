...
싱귤러 존(z ≥ `singular_z` = 0.50 m, 수직 최대 리치 부근)에서 **LIN scale 0.2 로
unstack 추출 시 손목 조인트가 256°/s 로 스파이크** — 225°/s 한계 초과
(`cup_stack/runtime.py` `_make_lin_slow_params` 주석). 선형 외삽 시 **scale ≈ 0.175 가
손목 한계 도달점** → 싱귤러 존 LIN 은 어떤 경우에도 **scale 0.1 초과 금지**.

## 2. 현재 운용값 (2026-06-12 기준)

| 프로파일 | 위치 | 용도 | vel / acc scale | 실속도 |
|---|---|---|---|---|
| Pilz PTP | `runtime.py _make_ptp_params` | pick 전 staging (joint-space) | 0.4 / 0.08 | 손목 ≤ 90°/s |
| Pilz LIN | `runtime.py _make_lin_params` | 하강·리프트·travel (z<0.50) | 0.1 / 0.08 | 100 mm/s, 0.18 m/s² |
| Pilz LIN slow | `runtime.py _make_lin_slow_params` | 싱귤러 존 (z≥0.50) | 0.05 / 0.04 | 50 mm/s |
| OMPL fallback | `runtime.py _make_ompl_params` | PTP 실패 폴백 | 0.4 / 0.08 | PTP 와 동일 거동 |
| native movel | server `domains/robot.py` (`vel/acc`) | `/move` staging | — | 150 mm/s / 300 mm/s² |
...
| ② LIN lift·travel, z<0.50 (pick 후) | 0.1 / 0.08 | 0.25 / 0.15 (250 mm/s) | **0.3 / 0.2** (300 mm/s, 0.45 m/s²) | 싱귤러 존 밖. 컵 파지 중이라 acc 가 슬립/출렁임 한계 |
